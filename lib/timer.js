...
export const myFormatSeconds = ({ start, end }) => {
...
export const myFormat = ({ start, end, finished=false }) => {
  const diff = Math.trunc((end - start) / 1000)
  const milli = Math.round( ((end-start)%1000)/10)
  const seconds = diff % 60
  let hours = 0
  let trueMinutes = 0
  let minutes = (diff - seconds) / MINUTE
  ...
  return finished
    ? `${pad(hours)}:${pad(minutes)}:${pad(seconds)}:${pad(milli)}`
